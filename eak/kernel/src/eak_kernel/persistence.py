...
class SQLiteEventStore:
    """Durable, append-only, tamper-evident reference event store.

    SQLite is a local/reference backend. The hash chain detects mutation of
    persisted execution history; it is not a substitute for an external audit
    log or distributed database in a production deployment.
    """
    ...
    def _backfill_legacy_chain(self) -> None:
        with self._lock, self._connect() as connection:
            ...
            for sequence, execution_id, event_type, payload_json, occurred_at, previous_hash, event_hash in rows:
                ...
                connection.execute(
                    "UPDATE eak_events SET occurred_at=?, previous_hash=?, event_hash=? WHERE sequence=?",
        ...
        with self._lock:
            ...
            try:
                ...
                row = connection.execute(
                    "SELECT event_hash FROM eak_events WHERE execution_id=? ORDER BY sequence DESC LIMIT 1",
                    (event.execution_id,),
                ).fetchone()
                ...
                connection.execute(
                    ...
                    INSERT INTO eak_events(
                        execution_id, event_type, payload_json, occurred_at, previous_hash, event_hash
                    ) VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (event.execution_id, event.type, payload, occurred_at, previous_hash, event_hash),
                )
                connection.commit()
            except Exception:
                connection.rollback()
