...
class CrossEncoderRerankerProvider:
    """Generic document reranker with explicit, unblended score semantics.

    Retrieval scores and cross-encoder scores are intentionally kept separate.
    Domain/evaluation layers decide how to interpret them; this provider never
    averages incompatible score spaces into a fake universal confidence value.
    """

    provider_id = "provider.cross-encoder-reranker"
    capability = "knowledge.rerank"

    def __init__(self, model: Any | None = None, *, model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2") -> None:
        self._model = model
    ...
    def rerank(
        ...
        *,
        ...
        raw_scores = self._model_or_default().predict(pairs)
